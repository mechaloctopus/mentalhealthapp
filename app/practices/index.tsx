...
const PRACTICES = [
  { id: 'breath', label: 'Breath', description: 'Box breathing, activation, extended exhale', glyph: '◯', color: colors.blue },
  { id: 'stillness', label: 'Stillness', description: 'Body scan, grounding, breath anchor', glyph: '◉', color: colors.teal },
  { id: 'sound', label: 'Sound', description: 'Mindful listening and sound meditation', glyph: '♪', color: colors.lavender },
  { id: 'sleep', label: 'Sleep', description: 'Wind-down practices for rest', glyph: '☽', color: colors.indigo },
  { id: 'loving-kindness', label: 'Loving-Kindness', description: 'Metta — expanding compassion in circles', glyph: '♡', color: colors.coral },
] as const;
    ...
    <SafeAreaView style={styles.root}>
      ...
      <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.list}>
          {PRACTICES.map((p) => (
            <Pressable key={p.id} onPress={() => router.push(`/practices/${p.id}` as any)}>
              <GlassCard style={styles.card}>
                <Text style={[styles.cardGlyph, { color: p.color }]}>{p.glyph}</Text>
...
const styles = StyleSheet.create({
  ...
  cardGlyph: { fontSize: 24, width: 36, textAlign: 'center' },
