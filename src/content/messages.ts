// 365 daily messages — affirmations, devotionals, quotes, breath cues,
// grounding, gratitude, stillness, resilience, and reflective thoughts.
// Deterministic: every day of the year maps to a stable, varied message.
...
export const TYPE_META: Record<MessageType, { label: string; accent: string; verb: string }> = {
...
const RHYTHM: MessageType[] = [
  'affirmation', 'breath', 'quote', 'gratitude', 'grounding', 'devotional',
  'affirmation', 'stillness', 'thought', 'resilience', 'quote', 'affirmation',
  'breath', 'devotional',
];
...
export function dayOfYear(date = new Date()): number {
  ...
  return Math.min(365, Math.max(1, Math.floor(diff / 86400000)));
}

export function todaysMessage(date = new Date()): DailyMessage {
  const idx = ((dayOfYear(date) - 1) % 365 + 365) % 365;
  return MESSAGES[idx]!;
}
